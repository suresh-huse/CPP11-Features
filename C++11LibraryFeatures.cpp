...
/*
	...
	//	and includes cv - qualification.Useful for generic code that need a reference(either 
	...
	//	An example of a function wrapper which just forwards other A objects to a new A object's 
...
//==============================================================
/*
	//	The std::thread library provides a standard way to control threads, 
	//  such as spawning and killing them.In the example below,
	//  multiple threads are spawned to do different calculations and 
...
/*
	...
	//	std::unique_ptr is a non - copyable, movable smart pointer that properly manages arrays and 
	//	STL containers.
	...
	//  A shared pointer holds a control block which has a few components such as the managed object 
	//	and a reference counter.
	//  All control block access is thread - safe, however, manipulating the managed object itself 
	//	is not thread - safe.
...
/*
	//	std::array is a container built on top of a C - style array.Supports common container 
...
/*
	...
	//  In order to achieve constant - time complexity, sacrifices order for speed by hashing 
...
/*
	//	std::make_shared is the recommended way to create instances of std::shared_ptrs due to 
	...
	//	we have introduced a leak here.
	//  With std::make_shared, we are given exception - safety:

	foo(std::make_shared<T>(), function_that_throws(), std::make_shared<T>());

	//  Prevents having to do two allocations.When calling std::shared_ptr{ new T{} }, 
...
/*
	//  C++11 introduces a memory model for C++, which means library support for threading and
	//	atomic operations.
	...
	std::launch::async | std::launch::deferred It is up to the implementation whether to 
	perform asynchronous execution or lazy evaluation.
